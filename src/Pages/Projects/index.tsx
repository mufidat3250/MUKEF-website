...
import { navData, charity, ongoingProjects, completedProject } from '../../data'
...
const Projects = () => {
  return (
    ...
    <NavTabs tabs={navData} tabIndex={2} />
      ...
      <Button title={'Donate now'} customStyle={'!bg-white !text-gray900 !rounded-[5px]'}/>
     </div>
          </div>          
         </div>
        <div className='ongoing--project'>
          ...
          <div className='project--container'>
            {
              ongoingProjects.map((project, index)=> <OngoingProject {...project} key={`ongoingProject${index}`} index={index}/>)
            }
          </div>
            <div className='grid place-items-end mt-[4.25rem] pb-[5.8125rem]'>
            <div className='w-[16.375rem]'>
           <Button title={'See all'} customStyle={'!bg-[#1D2319] rounded-[5px] text-white'}/>
          </div>
            </div>          
        </div>
          ...
          <div className='grid grid-cols-3  gap-[4.8562rem] pb-[4.2418rem]'>
            {
              completedProject.map((project, index)=> <CompletedProject {...project} key = {index}/>)
            }
          </div>
