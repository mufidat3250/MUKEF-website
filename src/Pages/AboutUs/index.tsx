...
const AboutUs = () => {
  return (
      ...
      <div className='prospect'>
        {
          prospects.map(({img, title, subtext}, index) => {
            return <div className='space-y-[1.5rem]' key={index}>
                      <div className='flex justify-center items-center space-x-[1.5rem] text-[#3D3D3D] text-4xl'>
                        <img src={img} alt="" />
                        <p>{title}</p>
                      </div>
                      <p className='max-w-[58.8125rem] text-[#606060] text-[1.375rem] leading-[190%]'>{subtext}</p>
            </div>
          })
        }
      </div>
   ...
   <div className=' py-[6.75rem] grid grid-cols-4 gap-[4.375rem]'>
        {
          cleric.map((c, index)=> <div key={index}>
            <img src={c.img} alt="" />
           <div className='flex justify-center items-center flex-col mt-3'>
           <p className='text-black text-[1.1679rem]'>{c.title}</p>
            <span className='text-[#727272] text-[0.9084rem]'>{c.subtext}</span>
           </div>
          </div>)
        }
      </div>
