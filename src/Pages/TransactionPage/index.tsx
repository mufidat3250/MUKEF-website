...
const TransactionPage = () => {
  ...
  const filteredResult = paymentHistorydata.filter((data) => data.paymentType.toLocaleLowerCase().includes(filtered.toLocaleLowerCase()) || data.amount.toLocaleLowerCase().includes(filtered.toLocaleLowerCase()) || data.status.toLocaleLowerCase().includes(filtered.toLocaleLowerCase()))



  const tableData = filteredResult
    .map((payment, index) => ({
      date: payment.date,
    ...
    <DashboardLayout>
      <div className='transaction--container'>
        ...
        <div className='flex justify-between pb-[2.8125rem] items-center'>
          <h1 className=' text-xl font-semibold text-[#100C2A]'>Payment history</h1>
          <div className='w-[18.75rem]'>
            <Input customStyle='bg-gray600 rounded-[5px]' icon={true} placeholder={'Search for anything...'} onChange={(e: any) => setFiltered(e.target.value)} />
          </div>
        </div>
